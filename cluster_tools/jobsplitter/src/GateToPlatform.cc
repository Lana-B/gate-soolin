...
int GateToPlatform::GenerateCondorSubmitfile()
{
	...
	submitFile<<endl;
        G4String outputLogDir=getenv("GC_DOT_GATE_DIR");

	for (G4int i=1;i<=nSplits;i++)
	{
		submitFile<<"Arguments      = "<<outputDir<<i<<+".mac"<<endl;                                              
		submitFile<<"Output         = "<<outputLogDir<<i<<+".out"<<endl;
		submitFile<<"Error          = "<<outputLogDir<<i<<+".err"<<endl;
		submitFile<<"Log            = "<<outputLogDir<<i<<+".log"<<endl;
